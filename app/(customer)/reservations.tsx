...
export default function MyReservations() {
  ...
  const loadReservations = async () => {
    try {
      ...
      const newStats: ReservationStats = {
        ...
        pending: reservationsData.filter(r => r.status === 'pending' || r.status === 'pending_email_verification').length,
  ...
  const getStatusText = (status: string) => {
    switch (status) {
      ...
      case 'pending_email_verification': return 'En attente (email)'
  ...
  const cancelReservation = async (reservation: ReservationWithDetails) => {
    Alert.alert(
      'Annuler la réservation',
      `Êtes-vous sûr de vouloir annuler votre réservation pour "${reservation.baskets.title}" ?`,
      [
  ...
  const filteredReservations = reservations.filter(reservation => {
    ...
    if (filter === 'pending') {
      return reservation.status === 'pending' || reservation.status === 'pending_email_verification'
    }
      ...
      >
        <View style={styles.headerContent}>
          ...
          <Text style={styles.headerSubtitle}>
            {stats.total} réservation{stats.total > 1 ? 's' : ''} • {stats.totalSpent} DA économisés
          </Text>
        </View>
      </LinearGradient>
      ...
      <View style={styles.statsSection}>
        <View style={styles.statsGrid}>
          ...
          <View style={styles.statCard}>
            ...
            <Text style={styles.statLabel}>Économies (DA)</Text>
          </View>
        </View>
      </View>
        ...
        {filteredReservations.length === 0 ? (
          <View style={styles.emptyState}>
            ...
            <Text style={styles.emptySubtitle}>
              {filter === 'all' 
                ? 'Vos réservations de paniers apparaîtront ici'
            ...
            <View key={reservation.id} style={styles.reservationCard}>
              <View style={styles.reservationHeader}>
                <View style={styles.reservationInfo}>
                  ...
                  <Text style={styles.reservationMerchant}>{reservation.baskets.merchants.name}</Text>
              ...
              <View style={styles.actionsContainer}>
                {(reservation.status === 'pending' || reservation.status === 'pending_email_verification') && !isExpired(reservation.expires_at) && (
                  <>
                    {reservation.status === 'pending' ? (
                      <TouchableOpacity
                        style={styles.actionBtn}
                        onPress={() => router.push(`/(customer)/reservation/${reservation.id}`)}
                      >
                        <Ionicons name="qr-code-outline" size={16} color={theme.colors.primary} />
                        <Text style={styles.actionBtnText}>Voir QR Code</Text>
                      </TouchableOpacity>
                    ) : (
                      <TouchableOpacity
                        style={styles.actionBtn}
                        onPress={() => router.push(`/(customer)/verify-reservation?reservationId=${reservation.id}`)}
                      >
                        <Ionicons name="mail-outline" size={16} color={theme.colors.primary} />
                        <Text style={styles.actionBtnText}>Vérifier par email</Text>
                      </TouchableOpacity>
                    )}
                ...
                {isExpired(reservation.expires_at) && (reservation.status === 'pending' || reservation.status === 'pending_email_verification') && (
        ...
        <View style={styles.quickActionsGrid}>
          ...
          >
            ...
            >
              ...
              <Text style={styles.quickActionText}>Découvrir paniers</Text>
            </LinearGradient>
          </TouchableOpacity>
